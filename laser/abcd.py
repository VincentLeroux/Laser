...
class Beampath:
    ...
    def add_thinlens(self, focal_length):
        ...
        new_rad, new_angle = np.dot(
            self.list_elements[-1].M, np.array([self.radius[-1], self.angle[-1]]))
    ...
    def remove_element(self, element_index):
        ...
        new_list = [elem for i, elem in enumerate(
            self.list_elements) if i not in element_index]
    ...
    def _recalculate(self):
        ...
        for idx, element in enumerate(self.list_elements):
            if not isinstance(element, Object):
                ...
                new_rad, new_angle = np.dot(self.list_elements[idx].M, np.array([
                                            self.radius[-1], self.angle[-1]]))
                self.radius = np.append(self.radius, new_rad)
                self.angle = np.append(self.angle, new_angle)
    ...
    def _plot_initialise(self, **kwargs):
        ...
        if self.list_elements:
            ...
            self.__ymax = np.max(
                np.abs(self.radius[np.isfinite(self.radius)])) * 1.5
...
class GaussianBeampath(Beampath):
    """
    Computes the beam radius and angle according to ABCD matrices using paraxial
    approximation and the complex Gaussian parameter. Units should be consistent (all mm or all m)
    """
    ...
    def add_thinlens(self, focal_length):
        ...
        """
        self.list_elements.append(Thinlens(focal_length))
        q1 = rth_to_q(self.radius[-1], self.angle[-1],
                      self.index[-1], self.wavelength)
    ...
    def add_interface(self, index, curvature=np.inf):
        ...
        new_rad, new_angle = q_to_rth(q2, self.index[-1], self.wavelength)
        self.radius = np.append(self.radius, new_rad *
                                np.sqrt(self.index[-2] / self.index[-1]))
        self.angle = np.append(self.angle, new_angle)
...
class Freespace(Element):
    ...
    def plot(self, ax, elem_pos, beam_path, **kwargs):
        ...
        if np.any(beam_path.index != beam_path.index[0]):
            ax.fill_between(pos_lim, [ymax, ymax], [
                            ymin, ymin], facecolor=col2, alpha=(1 - 1 / n))
    ...
    def plot(self, ax, elem_pos, beam_path, **kwargs):
        ...
        if np.isfinite(pos):
            ...
            ax.text(pos, ymax * (sc + tx), ('Image at\n' + prec %
                                            pos).rstrip('0').rstrip('.'), color=col, ha='center', va='bottom')
        else:  # Othewise, annote the figure that the image is at infinity
            ax.text(xmax - 0.01 * xrange, yrange * 0.01,
                    'Image', color=col, ha='right', va='bottom')
            ax.text(xmax - 0.01 * xrange, -yrange * 0.01,
                    'at \u221E \u2192', color=col, ha='right', va='top')
