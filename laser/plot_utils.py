...
def plot_zoom_inset(ax, xy1, xy2, loc=1, scale=(1., 1.), offset=(0.05, 0.05),
    ...
    # Get max z order to place the rectangle on top
    zmax = max([_.zorder for _ in ax.get_children()])
    r_data = mpl.patches.Rectangle(
        (xa, ya), dx, dy, fill=False, ec=color, zorder=zmax, lw=0.5)  # Build rectangle
    ...
    if 3 in edges:
        ax.plot([xa, ll[0]], [ya, ll[1]], c=color,
                alpha=alpha, lw=0.5, zorder=zmax + 1)
    if 4 in edges:
        ax.plot([xb, lr[0]], [ya, lr[1]], c=color,
                alpha=alpha, lw=0.5, zorder=zmax + 1)
    if 2 in edges:
        ax.plot([xa, ul[0]], [yb, ul[1]], c=color,
                alpha=alpha, lw=0.5, zorder=zmax + 1)
    if 1 in edges:
        ax.plot([xb, ur[0]], [yb, ur[1]], c=color,
                alpha=alpha, lw=0.5, zorder=zmax + 1)
    # Reset limits
    ax.set_xlim(limx)
    ax.set_ylim(limy)
    ...
    shadow = mpl.patches.Rectangle(
        of, lr[0] - ll[0], ul[1] - ll[1], color=color, ec=None, alpha=alpha, zorder=zmax)
    ax.add_patch(shadow)

    # Add axis inset
    nap = relative_inset_position(
        def_size, loc, (scalex, scaley), (offsetx - x_red, offsety - y_red))  # new axis position
    oap = ax.get_position()  # old axis position
    axz = plt.axes([oap.x0 + nap[0] * (oap.x1 - oap.x0), oap.y0 + nap[1] *
                    (oap.y1 - oap.y0), nap[2] * (oap.x1 - oap.x0), nap[3] * (oap.y1 - oap.y0)])
...
def cmap_nicify(cmap, transparent=False, idx_white=0, size_white=None):
    ...
    if not transparent:
        ...
        my_cmap_rgb[:, 0] = (1 - my_cmap_rgba[:, -1]) * \
            1 + my_cmap_rgba[:, -1] * my_cmap_rgba[:, 0]
        my_cmap_rgb[:, 1] = (1 - my_cmap_rgba[:, -1]) * \
            1 + my_cmap_rgba[:, -1] * my_cmap_rgba[:, 1]
        my_cmap_rgb[:, 2] = (1 - my_cmap_rgba[:, -1]) * \
            1 + my_cmap_rgba[:, -1] * my_cmap_rgba[:, 2]

        my_cmap_rgb[:, -1] *= 0
        my_cmap_rgb[:, -1] += 1
    if register and not idx_white and not transparent:
        mpl.cm.register_cmap(name=cmap.name + '_w',
                             cmap=ListedColormap(my_cmap_rgb))
    else:
...
def cmap_nicify_all():
    """
    Make the bottom of all colormaps white, and add them to the list
    of known mapswith the flag '_w'
    """
...
def custom_cubehelix(gamma=1.0, start=0.0, rotation=-0.5, hue=1.0):
    """
    Custom cubehelix colormap, can be called using 'custom_ch'.
    The reversed version is also accessible using '_r'
    """
    mpl.cm.register_cmap(name='custom_ch', data=chelix(
        gamma=gamma, s=start, r=rotation, h=hue))
    mpl.cm.register_cmap(name='custom_ch_r',
                         cmap=plt.get_cmap('custom_ch').reversed())
...
def color_darken(color, factor=0.33):
    ...
    """
    return tuple([c * factor for c in mpl.colors.to_rgb(color)])
